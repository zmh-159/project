...
public class UniformChangeModel extends General {
    static int count = 0;
    ...
    public double getUniform(double max) {
        return max * getCount();
    }

    public double getRandom(double max) {
        return getDoubleWith2(Math.round(max * 0.07 + Math.random() * (max * 0.1 - max * 0.07)));
    }
    ...
    public JSONObject getDynamic(JSONObject ids) {
        ...
        /** 交换区 **/
        double swapTotal = 2;
        double swapUsed = getUniform(getMinRandom(swapTotal));
        ...
        double memoryUsed = getUniform(getRandom(memoryTotal));
        ...
        double diskUsed = getUniform(getRandom(diskTotal));
...
//        res.put("cpu_idle", 100 - cpuUser - cpuSystem);
        ...
        res.put("uuid", uuid);
        res.put("gpu_utilization", getUniform(getRandom(100)));
        res.put("memory_utilization", getUniform(getRandom(100)));
        JSONObject cpus = new JSONObject();
        int cpu_nums = 2;
        for (int i = 0; i < cpu_nums; ++i) {
            JSONObject cpu = new JSONObject();
            cpu.put("cpu_id", ids.getJSONObject("cpu").getLong(i + ""));
            double temperature = getUniform(getRandom(45)) + 30;
            cpu.put("temperature", temperature);
            JSONObject cores = new JSONObject();
            for (int j = 0; j < 12; ++j) {
                JSONObject core = new JSONObject();
                double coreUser = getUniform(getRandom(100));
                double coreSystem = getUniform(getRandom(7));
                double coreIdle = 100 - coreUser - coreSystem;
                double coreFrequency = getUniform(getRandom(3600000));
        ...
        double cu = getUniform(getRandom(100));
        double cs = getUniform(getRandom(7));
        ...
        for (int i = 0; i < gpu_nums; ++i) {
            ...
            gpu.put("memory_utilization", getUniform(getRandom(100)));
            gpu.put("gpu_utilization", getUniform(getRandom(100)));
        ...
        for (int i = 0; i < disk_nums; ++i) {
            ...
            disk.put("read_speed", getUniform(getRandom(1024)));
            disk.put("write_speed", getUniform(getRandom(2048)));
            disks.put(i + "", disk);
        }
        ...
        for (int i = 0; i < network_nums; ++i) {
            ...
            network.put("down_speed", getUniform(getRandom(1024)));
            network.put("up_speed", getUniform(getRandom(1024)));
