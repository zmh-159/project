package org.parallel.server.webssh.service.impl;
...
import org.parallel.server.webssh.constant.ConstantPool;
import org.parallel.server.webssh.pojo.SSHConnectInfo;
import org.parallel.server.webssh.pojo.WebSSHData;
import org.parallel.server.webssh.service.WebSSHService;
...
public class WebSSHServiceImpl implements WebSSHService {
    ...
    public void recvHandle(String buffer, WebSocketSession session) {
        //指定获取ip
        JSONObject mesage = JSONObject.parseObject(buffer);
        if (mesage.getString("operate").equals("connect")) {
            Long nodeId = mesage.getLong("nodeId");
            String host = nic.getIp(nodeId);
            if (host == null) {
                logger.error("节点未能接入系统！");
                close(session);
                return;
            }
            mesage.put("host", host);
            mesage.remove("nodeId");
            buffer = mesage.toJSONString();
        }
        ...
        if (ConstantPool.WEBSSH_OPERATE_CONNECT.equals(webSSHData.getOperate())) {
            ...
            if (sshConnectInfo != null) {
                try {
                    ...
                    int rows = webSSHData.getRows();
                    int cols = webSSHData.getCols();
