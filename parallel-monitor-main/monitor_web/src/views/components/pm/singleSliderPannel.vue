<template>
  <ul :style="stjson1" class="slider-list">
    <li v-for="i in number" :key="i" :style="stjson2">
      <el-card :shadow="select[i]===true?always:hover" @click.native="selectChart(chartOptions[i-1].title.text)">
...
export default {
  ...
  created() {
    this.initArray()
    this.$set(this.select, 1, true)
    this.selectChart('1号CPU')
  },
  mounted() {
    var that = this
    setTimeout(function() {
  ...
  methods: {
    initChart() {
      ...
      for (var i = 0; i < this.number; i++) {
        ...
        this.$set(this.chartOptions[i].title, 'text', this.type[i])
        this.chart[i].setOption(this.chartOptions[i])
      }
    },
    setData(val) {
      var data = val.reduce(function(a, b) { return a.concat(b) })
      for (var i = 0; i < this.number; i++) {
        // console.log(this.chartOptions[i])
        // this.$set(this.chartOptions[i].title, 'text', data[i][1])
        this.chartOptions[i].series[0].data.splice(0, 1)
        this.chartOptions[i].series[0].data.push([(47 + this.count).toString(), data[i][0]])
        this.$set(this.typeData, i, data[i][1])
        this.chart[i].setOption(this.chartOptions[i])
      }
    },
    initArray() {
    ...
    selectChart(index) {
      var number = this.typeData.indexOf(index) + 1
      for (var i = 0; i < this.number; i++) {
        if (i !== number) {
          this.$set(this.select, i, false)
        }
      }
      this.$set(this.select, number, true)
