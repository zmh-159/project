<template>
  <div class="window-container" :style="{width:position.width,height:isMin?'30px':position.height,left:position.left,top:position.top}" @mousedown="makeWindowForground">
    <div class="window-title" @mousedown="startDrag">{{ title }}
      <button v-if="closable" class="window-btn window-close" @click="close">×</button>
      <!--      <button class="window-btn window-max" v-if="maxmize" @click="max">{{isMax?'□':'+'}}</button>-->
      <button v-if="minimize" class="window-btn window-min" @click="min">-</button>
    </div>
    <div v-show="!isMin" class="window-body">
      <slot />
...
export default {
  ...
  mounted() {
    this.$emit('send-winsize', this.position)
  },
  methods: {
    close: function() {
