...
export default {
  ...
  methods: {
    ...
    timeChange2() {
      if (this.value2 !== null) {
        this.resetArray(this.chartX2, this.chartY2)
        this.show2 = false
        var range = []
        ...
        pmNodeGpuState.queryTime(that.gpu_uuid, range).then(function(response) {
          ...
          if (data.length !== 0) {
            ...
            for (var i = 0; i < data.length; i++) {
              that.chartX2[i] = data[i]['createTime']
              that.chartY2[i] = data[i]['currentMemoryFrequency'].slice(0, data[i]['currentMemoryFrequency'].length - 4)
            }
            that.chartX2.reverse()
            that.chartY2.reverse()
          } else {
            // 数据值为空
        ...
        pmNodeGpuState.query(tthat.gpu_uuid).then(function(response) {
          ...
          // console.log(data)
          for (var i = 0; i < data.length; i++) {
            tthat.chartX2[i] = data[i]['createTime']
            tthat.chartY2[i] = data[i]['currentMemoryFrequency'].slice(0, data[i]['currentMemoryFrequency'].length - 4)
          }
          tthat.show2 = true
        })
      }
    ...
    realgpuFre1() {
      ...
      pmNodeGpuState.query(that.gpu_uuid).then(function(response) {
        ...
        var data = response.content
        console.log(data)
        if (that.loading[1] === true) { that.$set(that.loading, 0, false) }
        // console.log(response.content)
        var tran1 = []
        var tran2 = []
        for (var i = 0; i < data.length; i++) {
          tran1.push(data[i]['createTime'])
          tran2.push(data[i]['currentMemoryFrequency'].slice(0, data[i]['currentMemoryFrequency'].length - 4))
        }
