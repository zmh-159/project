<template>
  <div class="crud-opts">
    <span class="crud-opts-left">
      <!--左侧插槽-->
      <slot name="left" />
      <el-input v-if="crud.props.searchToggle" v-model="crud.query[searchContent]" clearable placeholder="请输入内容" class="input-with-select" @keyup.enter.native="crud.toQuery">
        <el-select slot="prepend" v-model="searchType" placeholder="请选择" @change="typeChange">
          <el-option
...
export default {
  ...
  data() {
    return {
      ...
      searchContent: this.searchOptions[0].value
    }
  },
  watch: {
    'crud.props.table'() {
